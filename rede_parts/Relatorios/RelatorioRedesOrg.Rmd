...
```{r rede ditadura, include=FALSE}
orgdecadenet <- yearfilter_net(personyrNet, 1970, 1984) %>% 
...
```{r betw ditadura, include=FALSE}
fullstats <- central_stats(orgdecadenet, "betweenness", lglnormal = F, "ditadura") %>% 
  bind_rows( central_stats(orgdecadenet, "betweenness", T, "ditadura")  ) %>% 
  bind_rows( central_stats(orgdecadenet, "betweenness", F, "ditadura", invertweight = T)  ) %>% 
  bind_rows( central_stats(orgdecadenet, "betweenness", T, "ditadura", invertweight = T)  ) %>% 
  bind_rows( central_stats(orgdecadenet, "eigenvector", T, "ditadura", invertweight = T)  ) %>% 
  bind_rows( central_stats(orgdecadenet, "eigenvector", T, "ditadura", invertweight = F)  )
```

```{r plotditadura, echo = F, message = F}
...
short_lab$short <- c("IBDF","MRE","SUDAM","EMBRAPA","PNUD-FAO-IBDF",
                     "MInter","IPesqMarinha","BNH","IBGE","MDIC",
                     "MAPA","MS","SP","Presid","SEPLAN",
                     "Senado", "Câmara","CNI","SANESP","Lloyd",
                     "DOCENAVE","Marinha", "Petrobrás", "RDEP", "Syndarma", 
                     "SUNAMAM", "UFRGS", "USP")
...
plot(multienrichjam::spread_igraph_labels(orgdecadenet),
     # layout = coords, 
...
# 
# legend("bottomleft",legend=temp_legend$label, 
...
# Including title
title("Organizations Network Ditadura (1970-1984)")
...
```{r rede sarney, include=FALSE}
orgdecadenet <- yearfilter_net(personyrNet, 1985, 1989) %>% 
  collapse_ind2org() %>% preparing_nodeappearance(30)

```

```{r betw sarney, include=FALSE}
fullstats <- fullstats %>% 
  bind_rows(central_stats(orgdecadenet, "betweenness", lglnormal = F, "sarney")) %>% 
  bind_rows( central_stats(orgdecadenet, "betweenness", T, "sarney")  ) %>% 
  bind_rows( central_stats(orgdecadenet, "betweenness", F, "sarney", invertweight = T)  ) %>% 
  bind_rows( central_stats(orgdecadenet, "betweenness", T, "sarney", invertweight = T)  ) %>% 
  bind_rows( central_stats(orgdecadenet, "eigenvector", T, "sarney", invertweight = T)  ) %>% 
  bind_rows( central_stats(orgdecadenet, "eigenvector", T, "sarney", invertweight = F)  )
```

```{r plotsarney, echo = F, message = F}
#.... Generating the figure ------
...
short_lab$short <- c("MRE", "EMBRAPA", "IBDF", "FBCN", "IBAMA",
                     "PNUD-FAO-IBDF", "SEMAM-PR", "Marinha")
...
# Including title
title("Organizations Network Sarney (1985-1989)")
...
```{r rede colloritamar, include=FALSE}
orgdecadenet <- yearfilter_net(personyrNet, 1990, 1994) %>% 
  collapse_ind2org() %>% preparing_nodeappearance(30)

# combinando os vértices
# Juntar SEMAM e MMA
V(orgdecadenet)$combine <- if_else(
  V(orgdecadenet)$org_limpo == "Secretaria de Meio Ambiente da Presidência",
  "Ministério do Meio Ambiente", #rename SEMAM to MMA
  V(orgdecadenet)$org_limpo
)
# Juntar MCT e SCT
V(orgdecadenet)$combine <- if_else(
  V(orgdecadenet)$org_limpo == "Secretaria de Ciência e Tecnologia da Presidência",
  "Ministério da Ciência e Tecnologia", #rename SCT to MCT
  V(orgdecadenet)$combine
)
...
orgdecadenet <- contract(orgdecadenet,
                      ...
                      vertex.attr.comb=list(#methods for combining attributes
                        weight="sum", #sum weights,
                        nparticipations = "sum",  #sum nparticipations
                        "last")) # keep last of others

orgdecadenet <- simplify(orgdecadenet)
```

```{r betw colloritamar, include=FALSE}
fullstats <- fullstats %>% 
  bind_rows( central_stats(orgdecadenet, "betweenness", lglnormal = F, "colloritamar") ) %>% 
  bind_rows( central_stats(orgdecadenet, "betweenness", T, "colloritamar")  ) %>% 
  bind_rows( central_stats(orgdecadenet, "betweenness", F, "colloritamar", invertweight = T)  ) %>% 
  bind_rows( central_stats(orgdecadenet, "betweenness", T, "colloritamar", invertweight = T)  ) %>% 
  bind_rows( central_stats(orgdecadenet, "eigenvector", T, "colloritamar", invertweight = T)  ) %>% 
  bind_rows( central_stats(orgdecadenet, "eigenvector", T, "colloritamar", invertweight = F)  )
```

```{r plotcolloritamar, echo = F}
# Creating layout
set.seed(123) #set seed to guarantee reproducibility of figure layout  
# coords <- layout_(orgdecadenet, with_fr())
# coords <- layout_(orgdecadenet, with_kk())
...
short_lab$short <- c("MRE", "EMBRAPA", "MCT", "MMA","IBAMA",
                     "ABPMEX", "SINDIMAD","DF","ABINEE", "USP",
                     "CSSEE", "IBPC", "VP", "MJ", "Marinha",
                     "Exército", "MEFP","MAPA", "Aeron.", "MS",
                     "MAçSoc","MME", "MTransp", "MPS","MTE",
                     "SEGOV","SDR","SGPR")
...
# Including title
title("Organizations Network Collor-Itamar (1990-1994)")
...
fullstats <- fullstats %>%
  bind_rows( central_stats(orgdecadenet, "betweenness", lglnormal = F, "fhc1")) %>% 
...
short_lab$short <- c("MRE", "MMA", "MS", "MCT", "EMBRAPA",
                     "MPOG", "ABIQUIM", "IBAMA", "AIMEX", "MAPA",
                     "CCSEE", "MDIC", "AEB", "SP", "MME", 
                     "Vitae", "CNI", "BNDES", "CEBDS", "Petrobrás",
                     "JdBotRio", "CNTBio", "INPI", "Senado","Câmara", 
                     "FBOMS", "WWF","UNICAMP","Marinha","UFPA")
...
# Including title
title("Organizations Network FHC1 (1995-1998)")
...
filtdecadenet <- induced_subgraph(orgdecadenet, 
                 V(orgdecadenet)[nparticipations  >= 2]) %>% preparing_nodeappearance(30)
...
spaced_coords <- multienrichjam::layout_with_qfr(filtdecadenet,
                                                 repulse = 3.5,
                                                 repulse.rad = (vcount(filtdecadenet)^3.2)-(2*10^5)
)

# Change labels: name for top5 between orgs
short_lab <- data.frame( long = V(filtdecadenet)$label) %>% filter(long != "")

short_lab$short <- c("MRE", "MAPA", "MCT", "MMA","MDIC",
                     "IBAMA", "MS", "SP","ABIQUIM", "MFundiaria",
                     "MIN","PE","UFPE", "SUDENE","AEB",
                     "INPE","MME","RJ","Vitae", "UFRJ",
                     "CEBDS","USP","MPOG","WBCSD","EMBRAPA",
                     "FBDS","Vale","Petrobrás","FBOMS","Marinha")

all_labs <- left_join(data.frame( long = V(filtdecadenet)$label),
                      short_lab)


V(filtdecadenet)$longlab <- V(filtdecadenet)$label
V(filtdecadenet)$label <- all_labs$short
...
plot(filtdecadenet,
     # layout = coords, 
     layout = spaced_coords,
     vertex.size = 5 + V(filtdecadenet)$invbetw*10,
     vertex.frame.color = "white",
     vertex.label.color = 'black',
     vertex.label.cex = .8,
     edge.width= 1# + E(filtdecadenet)$weight/15
)

# Including legend
tipos_presentes <- V(filtdecadenet)$type %>% unique
temp_legend <- filter(types, label %in% tipos_presentes)
...
# Including title
title("Organizations Network FHC2 (1999-2002) - 2 or more participations")
...
filtdecadenet <- induced_subgraph(orgdecadenet, 
                 V(orgdecadenet)[nparticipations  >= 5]) %>% preparing_nodeappearance(30)


# Creating layout
set.seed(123) #set seed to guarantee reproducibility of figure layout  
coords <- layout_(filtdecadenet, with_fr())
# coords <- layout_(orgdecadenet, with_kk())
# KK functions as a spring, more weight giving more distance. 
# FR does the opposite: attract when more weight, repels others
...
short_lab$short <- c("MMA","CEBDS","FBOMS","IBAMA","MAPA",
                     "MS", "FPalmares","Vitae", "CNI",
                     "MG","PR","MRE", "BNDES","SP",
                     "MCT","MDIC","Petrobras","MME", "Eletrobras",
                     "UFRJ","USP","PwC","EcoEnergy","TO",
                     "Embrapa","Congresso","AES","ICC","Marinha", "BR-IMO")
...
# legend("left",legend=temp_legend$label, 
#        col= temp_legend$color,
#        bty = "n", pt.cex = 1.2,
#        cex = 1, pch=16,
#        inset = -0.05,
...
# Including title
title("Organizations Network Lula1 (2003-2006)")
...
```{r plotlula2, echo = F, message = FALSE}
#.... Generating the figure ------
...
spaced_coords <- multienrichjam::layout_with_qfr(filtdecadenet,
                                                 repulse = 3.5,
                                                 repulse.rad = (vcount(filtdecadenet)^3.2)-(8*10^5)
)

# Change labels: name for top5 between orgs
short_lab <- data.frame( long = V(filtdecadenet)$label) %>% filter(long != "")

short_lab$short <- c("MMA","MRE","PR","Natura","MS",
                     "MAPA", "CONTAG","FUNBIO", "MDIC",
                     "MCT","CEBDS","IniVerde", "CNI","Petrobrás",
                     "EMBRAPA","FBOMS","IBAMA","ANVISA", "SP",
                     "BA","AM","MT","Senado","MG","FBMC",
                     "ACRIMAT","TO","Rio","UFPR","Câmara"
                     )
...
V(filtdecadenet)$longlab <- V(filtdecadenet)$label
V(filtdecadenet)$label <- all_labs$short
V(filtdecadenet)[id_org_unica == 416]$label <- "Marinha"
# V(filtdecadenet)[id_org_unica == 542]$label <- "BR-IMO"
...
plot(filtdecadenet,
     # layout = coords,
     layout = spaced_coords,
     vertex.size = 3 + V(filtdecadenet)$invbetw*8,
     vertex.frame.color = "white",
     vertex.label.color = 'black',
     vertex.label.cex = .6,
     edge.width= 1# + E(filtdecadenet)$weight/15
...
# Including title
title("Organizations Network Lula1 (2007-2010) - Orgs with 5+ participations")
...
spaced_coords <- multienrichjam::layout_with_qfr(filtdecadenet,
                                                 repulse = 3.5,
                                                 repulse.rad = (vcount(filtdecadenet)^5)-(8*10^5)
                                                 )

# Change labels: name for top5 between orgs
short_lab <- data.frame( long = V(filtdecadenet)$label) %>% filter(long != "")

short_lab$short <- c("MRE","MMA","IBAMA","ANVISA","MAPA",
                     "MS", "ICMBIO","EMBRAPA", "BNDES",
                     "MPesca","MPS","MTE", "SGPR","SPM",
                     "MCT","MEsp","MFaz","MME", "SECOM",
                     "SRI","MCOM","CC-PR","MinC","SPortos",
                     "GSI","MTur","SEDH","MDIC","MDS","Câmara"
                     )
...
V(filtdecadenet)[id_org_unica == 416]$label <- "Marinha"
V(filtdecadenet)[id_org_unica == 542]$label <- "BR-IMO"
V(filtdecadenet)[id_org_unica == 118]$label <- "CEBDS"
V(filtdecadenet)[id_org_unica == 737]$label <- "MEBB"
V(filtdecadenet)[id_org_unica == 318]$label <- "GTPS"
...
# Including title
title("Organizations Network Dilma1 (2011-2014) - Orgs with 5+ participations")
...
```{r plotdilmatemer, echo = F}
#.... Generating the figure ------

filtdecadenet <- induced_subgraph(orgdecadenet, 
                 V(orgdecadenet)[nparticipations  >= 3]) %>% preparing_nodeappearance(60)
...
spaced_coords <- multienrichjam::layout_with_qfr(filtdecadenet,
                                                 repulse = 3.5,
                                                 repulse.rad = (vcount(filtdecadenet)^4)-(2*10^5)
)

# Change labels: name for top5 between orgs
short_lab <- data.frame( long = V(filtdecadenet)$label) %>% filter(long != "")

short_lab$short <- c("MMA","MRE","IBAMA","MAPA","ANVISA",
                     "SP","MS", "SFB","IPEA", "IEChagas",
                     "AM","TO","MG", "Palmas","BH",
                     "Senado","Câmara","RS","Presid", "BNDES",
                     "RJ","PA","GO","MT","RO",
                     "RR","SaoPaulo","MFaz","MCT","TRF",
                     "EMBRAPA","FUNAI","AC", "EPE", "FBMC",
                     "GSI","Itaipu", "Congresso", "ANA", "APEX",
                     "Petrobrás","ICMBio","MDS","Marinha","Funbio",
                     "SabaráQuim","CNI","InstEcol","Votorantim","IBÁ",
                     "FIESP","ICONE","Vale","BR-IMO"
                     )
...
# V(filtdecadenet)[id_org_unica == 416]$label <- "Marinha"
# V(filtdecadenet)[id_org_unica == 542]$label <- "BR-IMO"
# V(filtdecadenet)[id_org_unica == 155]$label <- "Vale"
# V(filtdecadenet)[id_org_unica == 517]$label <- "Petrobrás"
...
# Including title
title("Organizations Network Dilma2-Temer (2015-2018) - Orgs with 3+ participations")
...
```{r plot-betwcentr-mandatos, echo = F}
fullstats %>% filter(weightinverted == T & normalized == T) %>% 
  filter(decade %in% c("ditadura","sarney","colloritamar", 
                       "fhc1", "fhc2", "lula1", "lula2", "dilma1", "dilmatemer")) %>% 
  filter(measure == "betweenness") %>%
  mutate(year = case_when(decade == "ditadura" ~ 1970,
                          decade == "sarney" ~ 1985,
                          decade == "colloritamar" ~ 1990,
                          decade == "fhc1" ~ 1995,
                          decade == "fhc2" ~ 1998,
                          decade == "lula1" ~ 2003,
                          decade == "lula2" ~ 2007,
  ...
  scale_x_continuous(name = NULL,
                     breaks = c(1970,1985,1990,1995,1998,2003,2007,2011,2015),
                     label = c("Ditadura","Sarney","Collor-Itamar",
                               "FHC1", "FHC2","Lula1","Lula2","Dilma1","Dilma-Temer")) +
  theme(legend.position = "none", ) +
  ggtitle("Betweenness centrality")
```

(Eigenvector, normalized and inverted weights)
```{r plot-eigencentr-mandatos, echo = F}
fullstats %>% filter(weightinverted == T) %>% 
  filter(decade %in% c("ditadura","sarney","colloritamar",
                       "fhc1", "fhc2", "lula1", "lula2", "dilma1", "dilmatemer")) %>% 
...
```{r tabela betwcentr mandatos, echo = F}
fullstats %>% filter(weightinverted == T & normalized == T) %>% 
  filter(decade %in% c("ditadura","sarney","colloritamar",
                       "fhc1", "fhc2", "lula1", "lula2", "dilma1", "dilmatemer")) %>% 
  filter(measure == "betweenness") %>% 
  mutate(org = if_else(is.na(org), "Average", org)) %>% 
  select(decade, org, value) %>% rename(decade = "mandato") %>% 
  knitr::kable(caption = "Betweenness centrality")
```


```{r tabela eigen mandatos, echo = F}
fullstats %>% filter(weightinverted == T) %>% 
  filter(decade %in% c("ditadura","sarney","colloritamar", 
                       "fhc1", "fhc2", "lula1", "lula2", "dilma1", "dilmatemer")) %>% 
  filter(measure == "eigenvector") %>% 
...
short_lab$short <- c("IBDF", "MRE", "SUDAM", "EMBRAPA", "UNDP-FAO-IBDF","MInt","IPqM",
                     ...
                     "Senate", "Deputies", "CNI", "SANESP", "Lloyd", "DOCENAVE",
...
# Juntar Marinha e Min da Marinha
V(orgdecadenet)$combine <- if_else(
  V(orgdecadenet)$org_limpo == "Ministério da Marinha",
  "Marinha do Brasil", #rename Ministério da Marinha to Marinha do Brasil
  V(orgdecadenet)$combine
...
# Change labels: name for top between orgs
short_lab <- data.frame( long = V(orgdecadenet)$label) %>% filter(long != "")
short_lab$short <- c("MRE", "EMBRAPA", "MCT", "MMA", "MS", "MPOG", "MAPA", "IBAMA", "SP", "ABIQUIM",
                     # rep("", 18))
...
filtdecadenet <- induced_subgraph(orgdecadenet, 
                 V(orgdecadenet)[nparticipations  >= 3]) %>% preparing_nodeappearance(30)
...
# Change labels: name for top between orgs
short_lab <- data.frame( long = V(filtdecadenet)$label) %>% filter(long != "")
short_lab$short <- c("MRE", "EMBRAPA", "MCT", "MMA", "MS", "MPOG", "MAPA", "IBAMA", "SP", "PE",
                     "USP","CCSEE","MDIC","AEB","MME","Vitae","CEBDS","INPE","VP","Marinha",
                     "MEFP","SGPR","CNPq", "MEC", "UNICAMP")
all_labs <- left_join(data.frame( long = V(filtdecadenet)$label),
                      short_lab)
...
```{r plot-net00s, echo = F, message = F}
# Creating layout
set.seed(123) #set seed to guarantee reproducibility of figure layout  
# coords <- layout_(orgdecadenet, with_fr())
# coords <- layout_(orgdecadenet, with_kk())
...
filtdecadenet <- induced_subgraph(orgdecadenet, 
                                  V(orgdecadenet)[nparticipations  >= 10]) %>% preparing_nodeappearance(45)
...
short_lab$short <- c("MRE","MAPA","MMA", "MDIC","MCT",
                     "CEBDS","FBOMS", "Senado", "IBAMA","MS",
                     "Vitae","CONTAG","CNI","MG","PR",
                     "BNDES","SP","AC","SC","Petrobrás",
                     "BA", "Vale","EMBRAPA","INPE", "MME","RJ",
                     "ICC","UFRJ","USP","FBMC","DF","TO","UNICA",
                     "AM","Câmara","MT","FIESP","BRACELPA",
                     "Presid","PA","BR-IMO","GSI","Marinha"
                     )
all_labs <- left_join(data.frame( long = V(filtdecadenet)$label),
                      short_lab)
...
V(filtdecadenet)$label <- all_labs$short
# Manually include relevant labels
V(filtdecadenet)[id_org_unica == 435]$label <- "MRE"
V(filtdecadenet)[id_org_unica == 442]$label <- "MMA"
V(filtdecadenet)[id_org_unica == 422]$label <- "MCT"
V(filtdecadenet)[id_org_unica == 421]$label <- "MAPA"
...
```{r plot-net10s, echo = F, message = F}
# Creating layout
set.seed(123) #set seed to guarantee reproducibility of figure layout  
# coords <- layout_(orgdecadenet, with_fr())
# coords <- layout_(orgdecadenet, with_kk())
...
# Change labels: name for top between orgs
short_lab <- data.frame( long = V(filtdecadenet)$label) %>% filter(long != "")
short_lab$short <- c("MRE","MMA", "IBAMA", "MAPA","MS","SP",
                     "ICMBIO","EMBRAPA","BNDES","MTE","SGPR","MCT",
                     "MFaz","SECOM","CC-PR","MinC","GSI","MTur",
                     "MDIC",#"MDS","MEC","MPOG","MD","RJ", #(qdo n labels é 25)
                     "Câmara"
                     )
...
plot(filtdecadenet,
     layout = multienrichjam::rotate_igraph_layout(layout = l, reflect = c("x","y")), 
     # layout = spaced_coords,
     vertex.size = 3 + V(filtdecadenet)$invbetw*10,
     # vertex.size = 3 + V(filtdecadenet)$newbetw/5,
     vertex.frame.color = "white",
...
```{r plot-filtnet10s-20plus, echo = F}
#.... Alternative figure: only orgs with 10+ participations -----
# To clear up the graph and better look at the community structure, 
# it might be more interesting to limit the organizations in the figure.
# Normally, this would be done by filtering vertex degree (n of connections).
...
short_lab <- data.frame( long = V(filtdecadenet)$label) %>% filter(long != "")
short_lab$short <- c("MRE","MMA", "IBAMA", "MAPA","MS","SP",
                     "EMBRAPA","BNDES","MCT","MFaz","GSI","MDS",
                     "RJ", "Senado","MG","SC","MT","AC","Câmara", "Rio",
                     "Petrobras","GO","RS","VP","AM","FIESP","USP","PR",
                     "MPF","UFRJ","FGV","CEBDS","CNI","UGT","CNA","Abril",
                     "Marinha","BR-IMO"
                     )
...
(Betweenness, normalized and inverted weights)
```{r plot-betwcentr-decada, echo = F}
fullstats %>% filter(weightinverted == T & normalized == T) %>% 
...
### Tabela das estatísticas por mandato presidencial
```{r tabela betwcentr decada, echo = F}
fullstats %>% filter(weightinverted == T & normalized == T) %>% 
  mutate(org = if_else(is.na(org), "Average", org)) %>% 
  filter(decade %in% c("70s","80s","90s","00s","10s")) %>% 
...
full_centr3 %>% mutate(org = if_else(is.na(org), "Average", org)) %>% 
  select(decade, org, value) %>% rename(decade = "year") %>% 
  knitr::kable(caption = "Normalized betweenness centrality, 4 year window network")
```

```{r tabela betwcentrality jan5, echo = F}
full_centr4 %>% mutate(org = if_else(is.na(org), "Average", org)) %>% 
  select(decade, org, value) %>% rename(decade = "year") %>% 
  knitr::kable(caption = "Normalized betweenness centrality, 5 year window network")
```

```{r tabela betwcentrality jan3, echo = F}
full_centr2 %>% mutate(org = if_else(is.na(org), "Average", org)) %>% 
  select(decade, org, value) %>% rename(decade = "year") %>% 
  knitr::kable(caption = "Normalized betweenness centrality, 3 year window network")
```
...
- Média de n orgs pós 2015 (incluso): `r freq_orgs %>% filter(ano >= 2015) %>% pull(total_orgs) %>% mean`

- Média de n orgs pós 2015 só clima (incluso): `r freq_orgs_tema %>% filter(ano >= 2015 & tema == "Clima") %>% pull(total_orgs) %>% mean`

- Média de n orgs biodiv pós 06 (incluso): `r freq_orgs_tema %>% filter(ano >= 2006 & tema == "Biodiversidade - CBD" & is.na(total_orgs) == F) %>% pull(total_orgs) %>% mean`
...
```{r facetsize, echo = FALSE, fig.width=11}
#plotando gráficos separados cf o quartil
ggplot(MRE_conf) +
...
facet <- bind_rows(MRE_conf, MRE_confgov)


ggplot(facet %>% filter(tipo_org_reduzido == "Governo federal MRE"), 
       aes(x=ano, y=percentual, color = Tema)) +
  geom_point(aes(size = deleg_size)) +
  geom_smooth() +
...
tipoorgs_decada %>% 
  ...
  ggplot(aes(fill = tipo_org_reduzido, values = 100*percentual)) +
